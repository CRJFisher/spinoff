...
from spinoff.agents import AgentInfo, _parse_frontmatter, discover_agents, check_configured_agents
...
class TestDiscoverAgents:
    ...
    def test_global_agents(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_project_agents(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        # Empty global dir
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    ...
    def test_mixed_agents_sorted(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        global_dir = tmp_path / "home" / ".claude" / "agents"
...
class TestCheckConfiguredAgents:
    def _setup_agents(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, names: list[str]) -> Path:
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
        project = tmp_path / "project"
        project_dir = project / ".claude" / "agents"
    ...
    def test_all_found(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_missing_agent(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_empty_config(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        project = self._setup_agents(tmp_path, monkeypatch, ["agent-a"])
