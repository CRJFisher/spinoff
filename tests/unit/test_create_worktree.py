...
class TestSanitizeTaskName:
    ...
    def test_strip_trailing_hyphens(self):
        assert sanitize_task_name("fix-") == "fix"
...
class TestWriteStartupScript:
    ...
    def test_has_claude_command(self, tmp_path):
        script = tmp_path / "start.sh"
    ...
    def test_has_exec_bash(self, tmp_path):
        script = tmp_path / "start.sh"
        write_startup_script(script, tmp_path / "wt", "", ["claude"])
        content = script.read_text()
        assert "exec bash" in content

    def test_with_build_includes_build_command(self, tmp_path):
        script = tmp_path / "start.sh"
    ...
    def test_without_build_omits_build(self, tmp_path):
        script = tmp_path / "start.sh"
        write_startup_script(script, tmp_path / "wt", "", ["claude"])
        content = script.read_text()
